    ...
    console.log(e)
          ...
          console.log('跳转其他小程序');
           if(item.IsDone==false){
            ...
            that.watchTaobo();
              ...
              my.navigateToMiniProgram({
                  appId: item.APPID,
                  path: item.AliAdvertisingLink + '?BrowseTime=' + item.BrowseTime+ '&id=' + item.Id + '&jl=' + item.RewardAmountTime+ '&id=' + item.Id + '&jl=' + item.RewardAmount,
                   extraData:{
        ...
        if(JumpType==12){
            ...
            if(item.AliAdvertisingLink){
                my.navigateTo({
                  url:item.AliAdvertisingLink + '?BrowseTime=' + item.BrowseTime+ '&id=' + item.Id + '&jl=' + item.RewardAmountTime+ '&id=' + item.Id + '&jl=' + item.RewardAmount
                })
            }else{
              my.alert({
