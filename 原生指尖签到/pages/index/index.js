...
Page({
  data:{
        onPen:0,
        runNum:0,
        resourceId: "AD_20210516000000100669",
				  // resourceId: "ad_tiny_2021002132668446_202103172200007828",
				userInfo:'',
				timer :'',
				animationData: {},
				topList:'',
  ...
  varsion(){
       request('/api/v1/Activity/GetVersionSetting','POST',{versionNumber:'1.0.73',appletsMark:'2021002132668446'}).then(res=>{
                    if(res.success){
                       console.log(res)
    ...
    if(options.userid){
        ...
        this.login(options.userid);
            ...
            my.getAuthCode({
              ...
                 success: (result) => {
              //      console.log(result);
                  request('/api/v2/Login/GetAliUserSilent/GetAliUserSilent','GET',{auth_code:result.authCode,appid:'2021002132668446'}).then(res=>{
                    if(res.success){
  ...
  login(userid){
         ...
         my.getAuthCode({
			...
			   success: (result) => {
      //      console.log(result);
           request('/api/v2/Login/GetAliUserSilent/GetAliUserSilent','GET',{auth_code:result.authCode,puserid:userid,appid:'2021002132668446'}).then(res=>{
             if(res.success){
                console.log('后端返回个人信息',res.data.access_token);
                my.setStorageSync({
                      key: 'token',
     ...
     request('/api/v5/Activity/GetTaskList','GET',{}
      ...
      //任务跳转页面
        ...
        if(JumpType==13){
            ...
            var link ='https://d.alipay.com/?scheme=' + encodeURIComponent('alipays://platformapi/startApp?appId=2021002132668446&page=pages%2Findex%2Findex%3Fyunmaid%3D'+taskId);  // 任务完成后续跳转, 浏览商品没有  
            //link = "https%3A%2F%2Fd.alipay.com%2F%3Fscheme%3Dalipays%3A%2F%2Fplatformapi%2FstartApp%3FappId%3D2021002132668446%26page%3Dpages%2Findex%2Findex%3Fyunmaid%3D123";
            link =encodeURIComponent('alipays://platformapi/startApp?appId=2021002132668446&page=pages%2findex%2findex%3fyunmaid%3d123');  // 任务完成后续跳转, 浏览商品没有  
            const userId = '12345677';  
            const userNick= '123XXX';
            const content= item.YMJsonString.Content;
            const appId = '2021002132668446'  // 小程序ID  
...
//默认增加金币/集分宝接口
  ...
  //流量位插件回调
    ...
    yunmaJupm(e){
        ...
        var link ='https://d.alipay.com/?scheme=' + encodeURIComponent('alipays://platformapi/startApp?appId=2021002132668446&page=pages%2Findex%2Findex%3Fyunmaid%3D123');  // 任务完成后续跳转, 浏览商品没有  
         //link = "https%3A%2F%2Fd.alipay.com%2F%3Fscheme%3Dalipays%3A%2F%2Fplatformapi%2FstartApp%3FappId%3D2021002132668446%26page%3Dpages%2Findex%2Findex%3Fyunmaid%3D123";
         link =encodeURIComponent('alipays://platformapi/startApp?appId=2021002132668446&page=pages%2findex%2findex%3fyunmaid%3d123');  // 任务完成后续跳转, 浏览商品没有  
        const userId = '12345677';  
        const userNick= '123XXX';
       ...
       const appId = '2021002132668446'  // 小程序ID  
        ...
        my.ap.navigateToAlipayPage({
         ...
         // https://market.m.taobao.com/app/retail-shop/rax-pi/pages/weex?wh_weex=true&pagePath=custom-1613618069764.htm&sellerId=2791424107&pageId=183375&shopId=0&bizCode=mcloud&extParams=%7B%22spm_cnt%22%3A%22mcloud.mc-183375%22%2C%22spma%22%3A%22mcloud%22%2C%22spmb%22%3A%22mc-183375%22%7D&nodeId=7fd2cf2d&contentId=null
          ...
          //&channel=test
          //&link=https%3A%2F%2Fd.alipay.com%2F%3Fscheme%3Dalipays%3A%2F%2Fplatformapi%2FstartApp%3FappId%3D2021002132668446%26page%3Dpages%252findex%252findex%253fyunmaid%253d123
