...
Page({
  data: {
    ...
    bottomLists:[],
    start:0,
    templateId1: 'bd8db3ae66e34a6fbcc0fbc6dde94413',
    templateId2: '',
    templateId3: '',
    modalShow:false,
    modalname:'',
  ...
  GetTaskList(){
      ...
      //点击签到广告
        ...
        if(JumpType==13){
            ...
            var link ='https://d.alipay.com/?scheme=' + encodeURIComponent('alipays://platformapi/startApp?appId=2021002139665742&page=pages%2Findex%2Findex%3Fyunmaid%3D'+taskId);  // 任务完成后续跳转, 浏览商品没有  
            //link = "https%3A%2F%2Fd.alipay.com%2F%3Fscheme%3Dalipays%3A%2F%2Fplatformapi%2FstartApp%3FappId%3D2021002139665742%26page%3Dpages%2Findex%2Findex%3Fyunmaid%3D123";
            link =encodeURIComponent('alipays://platformapi/startApp?appId=2021002139665742&page=pages%2findex%2findex%3fyunmaid%3d123');  // 任务完成后续跳转, 浏览商品没有  
            const userId = '12345677';  
            const userNick= '123XXX';
            const content= item.YMJsonString.Content;
            const appId = '2021002139665742'  // 小程序ID  
			 ...
			 AddRewardts(){
              ...
              that.setData({
                timets:''
							...
							that.$uma.trackEvent('renwu_01',{'success':1});
					}else{
						   my.showToast({
							content:res.msg,
							type:'none'
         ...
         var that=this;
          request('/api/v5/Activity/AddReward','POST',{taskId: this.data.Id,Mark:my.getStorageSync({ key: 'Mark' }).data,UserId:my.getStorageSync({ key: 'UserId' }).data}).then(res=>{
            if(res.success){
               ...
               console.log('this.data.topListS.usercount',this.data.usercount)
                ...
                my.uma.trackEvent('renwu_01',{'success':1});
                }else{
                    my.showToast({
                       type: 'none',
         ...
         if(item.IsAliAdvert==1&&this.data.alBanShow&&this.data.rwdh.length>0){
          ...
          if(item.IsDone==false){
              this.setData({
               ...
               my.uma.trackEvent('renwu_01',{'click':1})
          }
          this.setData({
            Id:item.Id
          })
