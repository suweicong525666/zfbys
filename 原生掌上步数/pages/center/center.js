...
Page({
  ...
  login(){
    ...
    my.getAuthCode({
      ...
         success: (result) => {
           console.log(result);
            request('/api/v2/Login/GetAliUser/GetAliUser','GET',{auth_code:result.authCode,puserid:this.data.userid?this.data.userid:'',appid:'2021002145669883'}).then(res=>{
             if(res.success){
  ...
  GetTaskList(){
     request('/api/v4/Activity/GetTaskList/GetTaskList','GET',{TaskArea:'wdymtsgg'},
				).then(syaligg=>{
					if(syaligg.success){
             this.setData({
  ...
  jump_myCoin(e){
     ...
     my.navigateTo({
      url: '/pages/child/myCoin/myCoin?index='+index
    });
  },
  jump_balance(){
    my.navigateTo({
